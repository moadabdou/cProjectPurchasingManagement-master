...
#define SALE_DETAILS "./layouts/employeer/sale_details.html"
    ...
    if (cJSON_GetObjectItem(user_data ,"role")->valueint == 0){ //normal  employeer
        ...
        if (strlen(query) == 0 || strlen(query) == 1){ // `` or /
            ...
            if(sales.content == NULL){
                printf("\n cant open one of the files :  %s %s %s", SALES_DATAFILE);
                SEND_ERROR_500;
                return;
            } 
            //parsing data  
        ...
        char  *side_content = "hey there starting the admin dashboard", *active = "0";
        printf(">> query : %s \n" , query );
        //render dashboard html 
        Props props[] ={
            {
